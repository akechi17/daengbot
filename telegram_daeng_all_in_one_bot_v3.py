...
class PriceListScraper:
    ...
    def generate(self, query: str, format_mode: str = "reseller") -> tuple[str, str]:
        ...
        # Special safe merge for Mobile Legends
        if normalize_name(query) in {"ml", "mlbb", "mobile legends"}:
            title_indo, text_indo = self.generate_single("Mobile Legends Indonesia", format_mode)
            title_glob, text_glob = self.generate_single("Mobile Legends Global", format_mode)
            merged_title = "Mobile Legends"
            merged_text = (
                "MOBILE LEGENDS\n\n"
                "INDONESIA\n\n"
                + remove_header(text_indo, title_indo)
                + "\n\nGLOBAL\n\n"
                + remove_header(text_glob, title_glob)
            ).strip()
            return merged_title, merged_text

        return self.generate_single(query, format_mode)
    ...
    def resolve_product_name(self, query: str) -> str:
        ...
        products = self.fetch_product_names()
        if not products:
            return query
    ...
    def fetch_product_names(self) -> list[str]:
        ...
        # Make safe user-facing list: collapse ML Indonesia+Global into one button
        seen: set[str] = set()
        cleaned: list[str] = []
        for name in names:
            norm = normalize_name(name)
            if norm in {"mobile legends indonesia", "mobile legends global"}:
                if "mobile legends" not in seen:
                    seen.add("mobile legends")
                    cleaned.append("Mobile Legends")
                continue
...
def list_menu(context: ContextTypes.DEFAULT_TYPE) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("Cari Game", callback_data="menu_list_search")],
...
async def send_list_text(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str) -> None:
    ...
    _title, text = state.scraper.generate(query, format_mode=mode)
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data == "menu_list_generate":
            ...
            await query.edit_message_text("Pilih Game", reply_markup=list_games_keyboard(games, 0))
            return

        if data.startswith("listgames_page|"):
            page = int(data.split("|", 1)[1])
            games = get_state(context).scraper.fetch_product_names()
            await query.edit_message_text("Pilih Game", reply_markup=list_games_keyboard(games, page))
            return
        ...
        if data == "menu_list_search":
            ...
            await query.edit_message_text(
                "Ketik nama game list harga.\nContoh: mlbb, freefire, roblox.\n\nTips: paling aman pakai Generate List lalu pilih dari tombol.",
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        if awaiting == "order_search_keyword":
            ...
            context.user_data["order_search_results"] = results
            if not results:
