...
logger = logging.getLogger("daeng_max_bot_v2")
...
FIELD_RULES = [
    (r"roblox.*via.*login", ["Username", "Password", "Backup Code"]),
    (r"roblox.*login", ["Username", "Password", "Backup Code"]),
    (r"roblox", ["Username"]),
...
def products_keyboard(products: list[Product], game: str, page: int, search_mode: bool = False) -> InlineKeyboardMarkup:
    ...
    nav = []
    if page > 0:
        nav.append(InlineKeyboardButton("<< Prev", callback_data=f"{'search_page' if search_mode else f'game|{game}'}|{page-1}" if search_mode else f"game|{game}|{page-1}"))
...
async def show_main(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str = "⚡️ Daeng Order Bot\n\nTinggal pencet tombol di bawah.") -> None:
