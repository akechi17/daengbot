...
logger = logging.getLogger("daeng_all_in_one_bot")
...
class PriceListScraper:
    ...
    def resolve_page(self, query: str) -> tuple[str, str]:
        ...
        debug = "; ".join(errors[:3]) if errors else "tidak ada kandidat slug yang valid"
        raise PriceBotError(f"Gagal menemukan halaman produk untuk '{query}'. Debug: {debug}")
    ...
    def resolve_product_name(self, query: str) -> str:
        ...
        for name in products:
            ...
            contains_bonus = 0.2 if norm in name_norm or name_norm in norm else 0.0
            scored.append((ratio + contains_bonus, name))
        ...
        close = get_close_matches(norm, list(exact_map.keys()), n=1, cutoff=0.5)
        if close:
...
async def callback_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    try:
        ...
        if data == "menu_list_search":
            ...
            await query.edit_message_text("Ketik nama game list harga.\nContoh: freefire, roblox, mobile legends global", reply_markup=InlineKeyboardMarkup([nav_row("menu_list_home")]))
            return
